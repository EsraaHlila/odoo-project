...
/** Search parent_id in Odoo by parent code */
...
async function mapRowToOdooChild(uid, row) {
  const parentId = await odooFindParentId(uid, row.parent_id);
  if (!parentId) {
    console.warn("Parent not found in Odoo for code:", row.parent_id);
  ...
  return {
    ...
    birth_date: row.birth_date || false,
    moyenne: row.moyenne != null ? row.moyenne : 0,
